...
int main(int argc, char* argv[]) {
	...
	//in case of Insufficient argurments, call this function and exit with failure
	if (argc != 3)
		usage(argv);

	fd = open(argv[1], O_RDONLY); ¹//arg to specify the Device to set speed
	if (fd < 0)
		error_exit("opening tty");

	speed = atoi(argv[2]); //set arbitrary speed
